...
function InputField({ type, name, placeholder, label, register, error }: InputFieldT) {
  return (
    <div className="my-2 flex flex-col">
      <label className="py-2 text-md text-black/50 font-mono font-medium">{label}</label>
      <input
        {...register(name)}
        className="px-3 py-2 rounded-md border border-black/30 placeholder:text-sm text-sm"
        type={type}
        name={name}
